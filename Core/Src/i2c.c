...
uint16_t NFC_IO_ReadMultiple(uint8_t Addr, uint8_t *pBuffer, uint16_t Length)
{
    ...
    // Ensure the I2C device is ready
    status = HAL_I2C_IsDeviceReady(&hi2c1, Addr, 3, NFC_I2C_TIMEOUT);
    if (status != HAL_OK)
    {
    ...
    status = HAL_I2C_Master_Receive(&hi2c1, Addr, pBuffer, Length, NFC_I2C_TIMEOUT);
    if (status != HAL_OK)
    {
        return NFC_IO_ERROR_TIMEOUT;
    }
...
uint16_t NFC_IO_WriteMultiple(uint8_t Addr, uint8_t *pBuffer, uint16_t Length)
{
    ...
    status = HAL_I2C_Master_Transmit(&hi2c1, Addr, pBuffer, Length, NFC_I2C_TIMEOUT);
    if (status != HAL_OK)
    {
        return NFC_IO_ERROR_TIMEOUT;
    }
